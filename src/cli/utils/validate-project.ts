...
export async function validateMCPProject() {
  try {
    ...
    if (
      !package_json.dependencies?.['@middesk/mcp-framework'] &&
      !package_json.devDependencies?.['@middesk/mcp-framework']
    ) {
      throw new Error(
        'This directory is not an MCP project (@middesk/mcp-framework not found in dependencies or devDependencies)'
      );
    }
  } catch (error) {
